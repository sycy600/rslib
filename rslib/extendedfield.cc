...
ExtendedField::ZechLogarithm::ZechLogarithm(unsigned int value)
  : value_(value) { }

bool ExtendedField::ZechLogarithm::isMinusInfinity() const {
  return std::numeric_limits<unsigned int>::max() == value_;
}

unsigned int ExtendedField::ZechLogarithm::getValue() const {
  return value_;
}
